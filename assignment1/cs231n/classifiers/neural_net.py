...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    C = len(b2)
    U = np.exp(scores) / np.repeat(denom, C).reshape((N,C))
    Q = np.zeros((N,C)); Q[range(N),y] = 1;
    df2 = dscores = U-Q                          # (N,C)
    grads['b2'] = np.sum(df2, axis=0) / N        # (C,)
    grads['W2'] = g1.T.dot(U-Q) / N  + reg*W2    # (H,C)
    dg1 = df2.dot(W2.T)                          # (N,H)
    df1 = (f1>0) * dg1                           # (N,H)
    grads['b1'] = np.sum(df1, axis=0) / N        # (H,)
    grads['W1'] = X.T.dot(df1) / N + reg*W1      # (D,H)
